...
4. MacBook 해상도를 선택하고 **PNG 저장** 또는 **배경화면으로 설정**을 누릅니다. 적용 대상은 **자동화**에서 선택한 디스플레이이며, 아직 선택하지 않았다면 앱 창이 있는 디스플레이입니다.
...
2. **자동화**에서 적용할 디스플레이를 확인하고 원하는 옵션을 켭니다. 캘린더 자동 교체 옵션을 켜면 선택한 캘린더의 이번 주 시간 지정 일정을 모두 가져와 **즉시 적용**합니다. 가져오기에서 제외하거나 수정했던 캘린더 일정도 이후 자동 갱신에서는 원본 내용으로 맞춥니다. 직접 추가한 일정은 유지합니다.
...
- 권한 철회·선택한 캘린더 삭제·조회 실패 시 이전 일정을 유지하며 상태를 표시합니다. 지정한 디스플레이가 빠지면 다른 화면을 대신 바꾸지 않고 재연결을 기다립니다. 날짜 표시만 갱신하는 경우에는 저장된 일정을 사용합니다.
...
- 작업은 `~/Library/Application Support/Serein/studio.json`에 자동 저장합니다. 선택한 캘린더·자동화 옵션·적용 기록도 함께 저장하며, 기존 버전의 저장 파일은 자동화를 끈 상태로 읽습니다. 적용한 PNG는 같은 디렉터리의 `Wallpapers`에 보존합니다. 같은 PNG는 파일을 재사용하고, 이전 파일은 다른 Space가 참조할 수 있어 유지합니다. 손상된 작업 파일은 별도의 recovery 파일로 보관한 후 새 작업을 저장합니다.
...
캘린더 변환·권한·비동기 상태 검증 85개와 자동화 79개는 합성 데이터와 테스트 provider로 수행했습니다. 렌더러 28개와 실제 Vision OCR을 포함한 47개 검사도 통과했습니다. 실제 계정의 EventKit 조회와 OS 배경화면 적용·로그인 항목 등록은 사용자의 기기에서 활성화한 뒤 확인해야 합니다. UI에서는 예시 캘린더 조회·수정·반영·3024×1964 PNG 저장을 검증했습니다.
