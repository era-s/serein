...
2. **자동화 → 배경화면 적용 범위**에서 **모든 데스크탑과 디스플레이**를 확인하고 원하는 옵션을 켭니다. 캘린더 자동 교체 옵션을 켜면 선택한 캘린더의 이번 주 시간 지정 일정을 모두 가져와 **즉시 적용**합니다. 가져오기에서 제외하거나 수정했던 캘린더 일정도 이후 자동 갱신에서는 원본 내용으로 맞춥니다. 직접 추가한 일정은 유지합니다.
